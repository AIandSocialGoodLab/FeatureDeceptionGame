...
class ApproxMILPBS:
    ...
    def evaluateSolution(self, trueGame):
        trueCfeatureWeights = trueGame.CfeatureWeights
        trueDfeatureWeights = trueGame.DfeatureWeights
        ewx = 0
        ewxu = 0
        for i in range(self.n):
            wx = 0
            for k in range(self.Kc):
                wx = wx + trueCfeatureWeights[k] * self.xcsol[(i, k)]
            for k in range(self.Kd):
                wx = wx + trueDfeatureWeights[k] * self.xdsol[(i, k)]
            ewx += np.exp(wx)
            ewxu += np.exp(wx) * self.nodes[i].u
        self.trueobj = ewxu / ewx
        return self.trueobj
