...
class LearnNN:
    ...
    def learn(self):
        ...
        self.GD_model = NN(self.K, 10, 20, True)
        self.learnGD()
        np.savetxt("historyGD"+str(self.n)+"_"+str(self.K)+"_"+str(self.seed)+"_"+str(self.Ntrain)+".csv", self.historyGD, delimiter=',')

        self.Ntrain = self.numData
        self.CF_model = NN(self.K, 10, 20, True)
        self.learnCF()
        np.savetxt("historyCF"+str(self.n)+"_"+str(self.K)+"_"+str(self.seed)+"_"+str(self.Ntrain)+".csv", self.historyCF, delimiter=',')
        return self.GD_model, self.CF_model
    ...
    def learnCF(self):
        ...
        self.historyCF = np.array([self.Ntrain, loss_test.item()/self.Ntest - loss_validation.item()/self.Ntest, TV_loss.item(), np.sum(np.abs(w - target_w))])
    ...
    def learnGD(self):
        ...
        self.historyGD = np.array([self.Ntrain, loss_test.item()/self.Ntest - loss_validation.item()/self.Ntest, TV_loss.item(), np.sum(np.abs(w - target_w))])
